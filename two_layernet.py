...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        relu_z2 = relu(X.dot(W1) + b1)
        scores = relu_z2.dot(W2) + b2  # N*C

        exp_scores = np.exp(scores)
        row_sum = exp_scores.sum(axis=1).reshape((N, 1))
        scores = exp_scores / row_sum
        ...
        data_loss = -1.0 / N * np.log(scores[np.arange(N), y]).sum()
        ...
        grads['W2'] = np.dot(relu_z2.T, der_scores)
        grads['b2'] = np.sum(der_scores, axis=0)

        hidden_lay = np.dot(der_scores, W2.T)  # (N,H)

        hidden_lay[relu_z2 <= 0] = 0

        grads['W1'] = np.dot(X.T, hidden_lay)
        grads['b1'] = np.sum(hidden_lay, axis=0)
