...
class MultiLayerPerceptron(nn.Module):
    def __init__(self, input_size, hidden_layers, num_classes):
        ...
        #layers += [torch.nn.Dropout(p = 0.2)] 
        for i in range(1, len(hidden_layers)):
            layers += [torch.nn.Linear(hidden_layers[i-1], hidden_layers[i])]
            layers += [torch.nn.LeakyReLU()] 
            #layers += [torch.nn.Dropout(p = 0.2)] 
        layers += [torch.nn.Linear(hidden_layers[-1], num_classes)]
        print(layers)
    ...
    def forward(self, x):
        ...
        out = self.layers(torch.flatten(x, start_dim=1))
        #_ , out = torch.max(scores, dim=1)
